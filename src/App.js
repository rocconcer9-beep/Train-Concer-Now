                      ...
                      <FormCard key={tpl.id}>
                        ...
                        {(editingTemplate.exercises||[]).map((ex,i)=>(
                          <div key={ex.id} style={{...S.card,marginBottom:6}}>
                            ...
                            <div style={{marginTop:6}}><label style={S.lbl}>Observacions</label><input style={S.inp} value={ex.observations||""} onChange={e=>setEditingTemplate(p=>({...p,exercises:p.exercises.map((ex2,j)=>j===i?{...ex2,observations:e.target.value}:ex2)}))}/></div>
                          </div>
                        ))}
